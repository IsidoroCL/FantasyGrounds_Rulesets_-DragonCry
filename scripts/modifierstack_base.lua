...
function onDrop(x, y, draginfo)
	...
	if dragtype == "number" then
		-- Get the number descriptive text
		local dragtext = draginfo.getDescription();
		
		-- Strip any names that were added
		dragtext = string.gsub(dragtext, "^(.*)-> ", "");
		
		-- Strip any roll totals that may have been added
		dragtext = string.gsub(dragtext, " %[%d+%]$", "");
	...
	elseif dragtype == "dice" or dragtype == "attack" or dragtype == "damage" or dragtype == "init" or dragtype == "skill" then
